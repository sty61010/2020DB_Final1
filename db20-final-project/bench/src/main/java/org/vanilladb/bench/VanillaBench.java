...
public class VanillaBench {
	...
	public void benchmark() {
		try {
			...
			boolean result = benchmarker.executeDatabaseCheckProcedure(conn, 0);
			...
			if (BenchmarkerParameters.PROFILING_ON_SERVER) {
				...
				benchmarker.startProfilingProcedure(getConnection(), 0);
			}
			...
			if (BenchmarkerParameters.PROFILING_ON_SERVER) {
				...
				benchmarker.stopProfilingProcedure(getConnection(), 0);
			}
	...
	private SutConnection getConnection() throws SQLException {
		return driver.connectToSut(client);
	}
}

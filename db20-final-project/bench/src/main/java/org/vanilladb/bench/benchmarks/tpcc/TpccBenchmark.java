...
public class TpccBenchmark extends Benchmark {
	...
	public void executeLoadingProcedure(SutConnection conn) throws SQLException {
		System.out.println("send the first spc");
		conn.callStoredProc(0, TpccTransactionType.SCHEMA_BUILDER.getProcedureId());
		System.out.println("send the second spc");
		conn.callStoredProc(0, TpccTransactionType.TESTBED_LOADER.getProcedureId());
	}
	...
	@Override
	public boolean executeDatabaseCheckProcedure(SutConnection conn, int rteID) throws SQLException {
		...
		case SP:
			result = conn.callStoredProc(rteID, txnType.getProcedureId(), params);
			break;
		}

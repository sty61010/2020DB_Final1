...
import org.calvin.server.Startup;
...
public class CalvinStartUp implements SutStartUp {
	private static Logger logger = Logger.getLogger(CalvinStartUp.class
			.getName());
	...
	public void startup(String[] args) {
		...
		Calvin.init(dbname, nodeID, getStoredProcedureFactory(), new PartionMgr());
		
		if (logger.isLoggable(Level.INFO))
			logger.info("VanillaBench server ready");
		
		try {
			SpStartUp.startUp(1099 + nodeID);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private CalvinStoredProcFactory getStoredProcedureFactory() {
		CalvinStoredProcFactory factory = null;

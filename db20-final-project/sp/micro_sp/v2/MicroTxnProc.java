...
//		}
		...
		int id = 0;
		for (CalvinRecord rec : reads.values()) {
			paramHelper.setItemName((String) rec.getVal("i_name").asJavaVal(), id);
			paramHelper.setItemPrice((double) rec.getVal("i_price").asJavaVal(), id++);
		}
		// set write keys
		for (int idx = 0; idx < paramHelper.getWriteCount(); idx++) {
			int iid = paramHelper.getWriteItemId(idx);
